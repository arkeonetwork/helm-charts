...
Bitcoin Cash is a reference client that implements the Bitcoin protocol for remote procedure call (RPC) use. It is also the second Bitcoin client in the network's history. Learn more about Bitcoin Core on the [Bitcoin Developer Reference docs](https://bitcoin.org/en/developer-reference).
...
By default, `bitcoind` will run as user `bitcoin` for security reasons and with its default data dir (`~/.bitcoin`). If you'd like to customize where `bitcoin-cash` stores its data, you must use the `BITCOIN_DATA` environment variable. The directory will be automatically created with the correct permissions for the `bitcoin` user and `bitcoin-cash` automatically configured to use it.

```sh
❯ docker run --env BITCOIN_DATA=/var/lib/bitcoin-cash --rm -it bitcoin-cash \
  -printtoconsole \
  -regtest=1
```
...
```sh
❯ docker run -v ${PWD}/data:/home/bitcoin/.bitcoin -it --rm bitcoin-cash \
  -printtoconsole \
  -regtest=1
```
...
```sh
❯ docker run --rm --name bitcoin-server -it bitcoin-cash \
  -printtoconsole \
  -regtest=1
```
...
```sh
❯ docker run -it --link bitcoin-server --rm bitcoin-cash \
  bitcoin-cli \
